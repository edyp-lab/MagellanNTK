...
mainapp_ui <- function(id, session, size = "300px") {
  ...
  includeCSS(file.path(
    system.file("www/css", package = "MagellanNTK"),
    "MagellanNTK.css"
  ))
...
mainapp_server <- function(
  ...
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(id,
      {
        if (is.null(workflow.path())) {
          stop("workflow.path is NULL. Abort...")
        }
        
        if (is.null(workflow.name())) {
          stop("workflow.name is NULL. Abort...")
        }
        ...
        if (session$userData$runmode == "process") {
          rv.core$process.name <- unlist(strsplit(
            rv.core$workflow.name, "_"
          ))[2]
          rv.core$pipeline.name <- unlist(strsplit(
            rv.core$workflow.name, "_"
          ))[1]
        }
    ...
    output$WF_Name_UI <- renderUI({
      ...
      h4(paste0("Workflow: ", rv.core$workflow.name),
    ...
    output$Dataset_Name_UI <- renderUI({
      ...
      h4(paste0("Dataset: ", rv.core$current.obj.name),
        style = "background-color: lightgrey;"
      )
    })
    ...
    observe_result_open_dataset <- observeEvent(
      rv.core$result_open_dataset()$trigger,
      ignoreInit = TRUE,
      ignoreNULL = TRUE,
      {
    ...
    observe({
      rv.core$result_convert <- nav_single_process_server(
        id = paste0(
          unlist(strsplit(rv.core$workflow.name, "_"))[1],
          "_Convert"
        ),
        dataIn = reactive({
          NULL
    ...
    output$open_convert_dataset_UI <- renderUI({
      nav_single_process_ui(ns(paste0(unlist(
        strsplit(rv.core$workflow.name, "_")
      )[1], "_Convert")))
    })

    observe_result_convert <- observeEvent(
    ...
    observe_result_run_workflow <- observeEvent(
      rv.core$result_run_workflow$dataOut()$value,
      {
    ...
    output$ReleaseNotes_UI <- renderUI({
      ...
      MagellanNTK::mod_release_notes_server(
        "rl",
        rv.core$funcs$URL_ReleaseNotes
      )
...
mainapp <- function(usermod = "user") {
  ...
  server <- function(input, output, session) {
    mainapp_server("main",
      ...
      workflow.path = reactive({
        system.file("workflow/PipelineDemo",
          package = "MagellanNTK"
        )
      }),
      usermod = usermod
