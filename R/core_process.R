  ...
  moduleServer(id, function(input, output, session) {
    ...
    output$DoBtn <- renderUI({
      ...
      enable.do.Btns <- unname(
        ...
        unname(
          rv$steps.status[len]
        ) != stepStatus$SKIPPED && (!is.null(dataIn())) &&
    ...
    output$DoProceedBtn <- renderUI({
      ...
      if (length(names(rv$steps.status)) == 1 &&
        ("Save" == names(rv$steps.status) ||
          "Description" == names(rv$steps.status))
      ) {
        enable.doProceed.Btns <- FALSE
      }
      ...
      ignoreNULL = TRUE,
      {
        ...
        rv$proc <- do.call(
          ...
          list(
            ...
            remoteReset = reactive({
              rv$rstBtn() + remoteReset() + remoteResetUI()
            }),
    ...
    output$testTL <- renderUI({
      ...
      div(
        style = paste0(
          ...
          "px;",
          "padding-left: ", default_layout$padding_left_process_sidebar,
          "px;",
    ...
    observeEvent(dataIn(), ignoreNULL = FALSE, ignoreInit = FALSE, {
      ...
      if (is.null(session$userData$funcs$GetHistory)) {
        session$userData$funcs$GetHistory <- "MagellanNTK::GetHistory"
      }
      parts_GetHistory <- strsplit(session$userData$funcs$GetHistory, "::",
        fixed = TRUE
      )[[1]]
      rv$history <- do.call(
    ...
    observeEvent(rv$proc$dataOut()$trigger,
      ...
      ignoreInit = TRUE,
      {
        ...
        # if (rv$current.pos == 1) {
        if ((rv$proc.id == "Description" || rv$current.pos == 1) &&
          (rv$proc.id != "Save")) {
    ...
    observeEvent(is.skipped(), ignoreNULL = FALSE, ignoreInit = TRUE, {
      if (isTRUE(is.skipped())) {
        setNames(rep(stepStatus$SKIPPED, length(rv$steps.status)), 
                 rv$steps.status)
      } else {
        setNames(rep(stepStatus$UNDONE, length(rv$steps.status)), 
                 rv$steps.status)
        rv$steps.enabled <- Update_State_Screens(
          is.skipped = is.skipped(),
          is.enabled = is.enabled(),
...
nav_process <- function() {
  ...
  ui <- fluidPage(
    tagList(
      fluidRow(
        column(width = 2, actionButton("simReset", 
                                       "Remote reset",
                                       class = "info")),
        column(width = 2, actionButton("simEnabled", 
                                       "Remote enable/disable", 
                                       class = "info")),
        column(width = 2, actionButton("simSkipped", 
                                       "Remote is.skipped", 
                                       class = "info"))
      ),
      hr(),
