...
open_dataset_server <- function(id,
  ...
  moduleServer(id, function(input, output, session) {
    ...
    core <- paste(
      ...
      MagellanNTK::Get_Code_for_ObserveEvent_widgets(
        names(widgets.default.values)
      ),
    ...
    observe({
      ...
      cond1 <- rv.widgets$chooseSource == "customDataset" &&
        !is.null(rv.widgets$file$datapath)
      cond2 <- rv.widgets$chooseSource == "packageDataset" &&
        !is.null(rv.widgets$pkg) && rv.widgets$pkg != ""
        ...
        rv.custom$dataRead <- tryCatch(
          {
            ...
            rv.custom$name <- unlist(strsplit(rv.widgets$file$name,
              split = ".", fixed = TRUE
            ))[1]
            get(rv.custom$name)
          },
