...
#' @param is.enabled A `boolean`. This variable is used as a remote command to
#' specify if the corresponding module is enabled/disabled in the calling module
#' of upper level.
#' For example, if this module is disabled, then this variable is set to TRUE.
#' Then, all the widgets will be disabled. If not, the enabling/disabling of
#' widgets is deciding by this module.
#'
...
#' process
#' @param verbose A `boolean` to indicate whether to turn off (FALSE) or ON
#' (TRUE) the verbose mode for logs.
...
#' be send to the caller in case of reseting the process. This is usefule for
#' example for the Convert process
#'
#' @return A shiny App
#'
#' @examples
    ...
    shiny::absolutePanel(
      ...
      actionButton(ns("btn_eda_singleProcess"),
        ...
        style = "padding: 0px; margin: 0px; border: none;
          ...
          font-size: 12px; z-index: 999999999999;"
      )
    )
  )
}
...
nav_single_process_server <- function(
  ...
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$btn_eda_singleProcess, {
      ...
      do.call(
        getExportedValue(parts_infos_dataset[1], paste0(
          parts_infos_dataset[2],
          "_server"
        )),
        list(
          id = "eda1",
      ...
      parts_history_dataset <- strsplit(session$userData$funcs$history_dataset,
        "::",
        fixed = TRUE
      )[[1]]
      do.call(
        getExportedValue(
          parts_history_dataset[1],
          paste0(parts_history_dataset[2], "_server")
        ),
        list(
          id = "eda2",
          dataIn = reactive({
      ...
      parts_view_dataset <- strsplit(session$userData$funcs$view_dataset, "::",
        fixed = TRUE
      )[[1]]
      do.call(
        getExportedValue(parts_view_dataset[1], paste0(
          parts_view_dataset[2],
          "_server"
        )),
        list(
          id = "eda3",
          ...
          shiny::tabsetPanel(
            ...
            shiny::tabPanel(
              ...
              do.call(
                getExportedValue(
                  parts_history_dataset[1],
                  paste0(parts_history_dataset[2], "_ui")
                ),
                list(id = ns("eda2"))
              )
            ),
            shiny::tabPanel(
              ...
              do.call(
                getExportedValue(
                  parts_view_dataset[1],
                  paste0(parts_view_dataset[2], "_ui")
                ),
                list(id = ns("eda3"))
              )
            )
    ...
    output$process_panel_ui_process <- renderUI({
      shiny::absolutePanel(
        ...
        width = "350px",
        # sidebar style, containing info for the process
        # (timeline, buttons, parameters)
        style = paste0(
          "position : absolute; ",
          "background-color: ",
          MagellanNTK::default_theme(
            session$userData$usermod
          )$bgcolor_pipeline_sidebar, "; ",
          "border-right: ", default_layout$line_width, "px solid ",
          default_layout$line_color, ";",
          "height: 100vh;"
        ),
        div(
        ...
        div(
          id = ns("div_process_panel_ui_process_ui"),
      ...
      div(
        ...
        display: flex; ",
          ...
          "background-color: ", MagellanNTK::default_theme(
            session$userData$usermod
          )$bgcolor_process_timeline, ";",
          "border-top: ", default_layout$line_width, "px solid ",
          default_layout$line_color, ";"
        ),
    ...
    output$DoBtn <- renderUI({
      ...
      enable.do.Btns <- unname(
        rv$steps.status[rv$current.pos]
        ...
        unname(rv$steps.status[len]) != stepStatus$SKIPPED &&
        (!is.null(dataIn())) &&
    ...
    output$testTL <- renderUI({
      ...
      div(
        style = paste0(
          ...
          "padding-bottom: ", default_layout$padding_bottom_process_sidebar,
          "px;",
          "padding-left: ", default_layout$padding_left_process_sidebar, "px;",
          "border-bottom: ", default_layout$line_width, "px solid ",
          default_layout$line_color, ";"
        ),
    ...
    observeEvent(dataIn(), ignoreNULL = FALSE, ignoreInit = TRUE, {
      ...
      # When executing a single process, the strategy for the dataset
      # is as follows:
      # The results of intermediate processes are not taken into account;
      # only the final result is considered.
      # To do this, all intermediate datasets are deleted, and only
      ...
      parts_SetHistory <- strsplit(session$userData$funcs$SetHistory, "::",
        fixed = TRUE
      )[[1]]
      rv$temp.dataIn[[1]] <- do.call(
        getExportedValue(parts_SetHistory[1], parts_SetHistory[2]),
        list(
          obj = rv$temp.dataIn[[1]],
    ...
    observeEvent(input$DoProceedBtn, ignoreInit = FALSE, ignoreNULL = TRUE, {
      ...
      rv$btnEvents <- paste0(
        names(rv$steps.status)[rv$current.pos],
        "_Do_Proceed_", input$DoProceedBtn
      )
    ...
    RefineProcessStatus <- function(history, steps.status) {
      ...
      if (length(steps.status) > 1) {
        ...
        names(steps.status) == "Description") {
        steps.status[1] <- stepStatus$VALIDATED
    ...
    observeEvent(rv$current.pos, ignoreInit = FALSE, {
      if (length(rv$config@steps) == 1) {
        ...
        shinyjs::toggleState(
          id = "nextBtn",
          condition = rv$current.pos < length(
            rv$config@steps
          )
        )
      }
      ...
      enable.do.Btns <- enable.doProceed.Btns <- unname(
        rv$steps.status[rv$current.pos]
      ) != stepStatus$VALIDATED &&
        unname(rv$steps.status[len]) != stepStatus$VALIDATED
...
nav_single_process <- function() {
  ...
  proc.name <- "PipelineDemo_Preprocessing"
