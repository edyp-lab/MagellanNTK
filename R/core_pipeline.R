...
nav_pipeline_server <- function(
  ...
  moduleServer(id, function(input, output, session) {
    ...
    rv <- reactiveValues(
      ...
      # corresponding step (it is one on the parameter for the server()
      # function)
    ...
    output$pipeline_panel_ui <- renderUI({
      shiny::absolutePanel(
        ...
        div(
          id = ns("div_EncapsulateScreens_ui"),
    ...
    output$pipeline_tl_btn_ui <- renderUI({
      div(
        style = paste0(
          "padding-left: ", default_layout$left_pipeline_timeline,
          "px; margin-top: -15px;",
          "width: 100%;",
        ...
        fluidRow(
          ...
          column(
            width = 2,
            div(
              id = ns("div_btns_pipeline_ui"),
              style = paste0(
                "display: inline-block;",
                "vertical-align: middle; "
              ),
              div(
                id = ns("div_btns_start_ui"), style = "display: inline-block;",
                uiOutput(ns("startBtnUI"))
              ),
              div(
                id = ns("div_btns_prev_ui"), style = "display: inline-block;",
                uiOutput(ns("prevBtnUI"))
              ),
              div(
                id = ns("div_btns_next_ui"), style = "display: inline-block;",
    ...
    observeEvent(input$btn_eda, {
      ...
      parts_infos_dataset <- strsplit(
        session$userData$funcs$infos_dataset, "::",
        fixed = TRUE
      )[[1]]
      do.call(
      ...
      parts_history_dataset <- strsplit(
        session$userData$funcs$history_dataset, "::",
        fixed = TRUE
      )[[1]]
      do.call(
        getExportedValue(
    ...
    observeEvent(GetValuesFromChildren()$triggers, ignoreInit = TRUE, {
      ...
      if (sum(is.na(triggerValues)) == length(triggerValues) ||
        ...
        ind.processHasChanged <- which(
          names(rv$config@steps) == processHasChanged
        )

        # Get the new value
        newValue <- tmp.return[[processHasChanged]]$dataOut()$value

        if (is.null(newValue)) {} else if (is.numeric(newValue) &&
          newValue == stepStatus$RESETED) {
          # A process has been reseted
          lastValidated <- GetMaxValidated_BeforePos(
            pos = ind.processHasChanged, rv = rv
          )
...
nav_pipeline <- function() {
  ...
  ui <- fluidPage(
    tagList(
      fluidRow(
        column(width = 2, actionButton("simReset", "Remote reset",
          class = "info"
        )),
        column(width = 2, actionButton("simEnabled", "Remote enable/disable",
          class = "info"
        ))
      ),
      hr(),
