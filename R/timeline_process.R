...
#' sub-steps (See the slot sub-steps in the config object). Each element
#' indicates if the corresponding step is enabled (TRUE) or (DISABLED)
#'
#' @return A shiny app
#'
#' @name timelines
...
timeline_process_ui <- function(id) {
  ns <- NS(id)
  fpath <- system.file("www/sass", "process_timeline.sass",
    package = "MagellanNTK"
  )
