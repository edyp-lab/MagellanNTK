...
#' 
#' In this example, `Demo_Clustering_ui()` and `Demo_Clustering_server()` define
#' the code for the process `Demo_Clustering` which is part of the pipeline called `Demo`.
#' 
#' @examples
#' if (interactive()){
#' library(MagellanNTK)
#' data(lldata, package = 'MagellanNTK')
#' path <- system.file('workflow/Demo', package = 'MagellanNTK')
#' shiny::runApp(proc_workflowApp("Demo_Clustering", path, dataIn = lldata))
#' }
#' 
#' 
NULL
...
#' @export
#' 
Demo_Clustering_conf <- function(){
  MagellanNTK::Config(
    fullname = 'Demo_Clustering',
    mode = 'process',
    steps = c('Clustering'),
    mandatory = c(FALSE)
  )
...
#' @export
#'
Demo_Clustering_ui <- function(id){
  ns <- NS(id)
}
...
#' @export
#' 
Demo_Clustering_server <- function(id,
